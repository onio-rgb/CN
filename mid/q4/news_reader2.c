...
{
    printf("Paused reading : News reader 2\n");
    pause();
}
void resume_exec(int sig)
{
    printf("Resumed reading : News reader 2\n");
}
void handleLiveTelecast()
{

    int fd = fileno(popen("pidof ./news1", "r"));
    char s[1000];
    read(fd, &s, 1000);
    int X = atoi(s);
    printf("%d\n",X);
...
int main()
{
    ...
    screen_addr.sin_port = htons(9001);
    ...
    while (1)
    {
        ...
        sprintf(add_reader_name, "%s From news reader 2\n", buff);
